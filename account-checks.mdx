...
There are many other things you will likely need to consider to fully secure your program, but these are the most common footguns you will come up against. Having a checklist is a great way to make sure you have taken all of these things into account. Upon finishing up all of htese checks, we can t
...
Contrarily, when prioritizing for peak performance, if your instruction is not constrained by the number of accounts it takes in, it is actually preferable to pass in Sysvar accounts instead of invoking the Sysvar syscalls to save CUs. This is because the base cost of invoking Sysvar syscalls is `100 CUs`, however when paired with an efficient entrypoint like [nostd-entrypoint](https://github.com/cavemanloverboy/solana-nostd-entrypoint) or [pinocchio](https://github.com/febo/pinocchio), passing in the account can actually achieve the same result for roughly `10CUs` or so.
