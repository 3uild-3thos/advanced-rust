...
```
 ...
 the Program ID is known, it is possible to omit the executable check, as checking for the correct Program ID already enables us to know that it is executable. There are also runtime errors that will occur when attempting to execute instructions against a non-executable account.
