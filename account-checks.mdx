...
- mutating the `lamports` balance of the account without CPI
- mutating the `data` of the account in your program

   when we are mutating their data or lamports, as if they do not belong to your program, the runtime will throw a privelege escalation error, causing your transaction to fail.
