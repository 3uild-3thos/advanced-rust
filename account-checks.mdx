...
When initializing new accounts, we need to make sure that the account is not already initialized. Naively, we may initially think that checking if the account is rent excempt would be a way to do this, however as we can send lamports to any account we want, this is not sufficient, as it requires us to .
