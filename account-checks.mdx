...
In anchor, we solve for these checks with [Account&lt;T&gt;](https://github.com/coral-xyz/anchor/blob/master/lang/src/accounts/program.rs#L30). This takes in a generic type that implements the `Owner` trait which is typically automatically set to `crate::ID` by the `#[account]` macro. This will mean Account will automatically use your elow is an example using it in conjunction with the `Token` program type:
