...
In anchor, we solve for these checks with [Account&lt;T&gt;](https://github.com/coral-xyz/anchor/blob/master/lang/src/accounts/program.rs#L30). This takes in a generic type that implemented the `Owner` t Below is an example using it in conjunction with the `Token` program type:
