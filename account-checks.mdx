...
Account checks are one of the most important important things we must consider to secure Solana programs. Here is a list of common account checks you should consider performing, along with _how_ and _why_.
...
In anchor, we solve for these checks with `Program<T>`. Below is an example using it in conjunction with the `Token` program type:
...
In Native Rust, we would handle that like so:

\`\`\`rs

assert!(&token\_[program.is](http://program.is)\_executable());

assert_eq!(&spl_token::ID, &token_program.key);

\`\`\`

In cases like this where the Program ID is known, it is possible to omit the executable check, as checking for the correct Program ID already enables us to know that it is executable. There are also runtime errors that will occur when attempting to execute instructions against a non-executable account.
...
When prioritizing security, it is often best to access Sysvar data directly from the sysvar syscalls instead of passing in an account, as this closes off all Sysvar-account-based attack vectors.

**Advanced:**

When prioritizing for performance, it is typically preferable to pass in Sysvar accounts instead of invoking the syscalls. This is because the base CU cost of invoking Sysvar syscalls is `100 CUs`, however when paired with an efficient entrypoint, passing in the account can actually achieve the same result for `~10CUs`.
...
assert!([maker.is](http://maker.is)\_signer);
