...
Uninitialized accounts require us to cehck that the account is not already initialized. This means checking that the account:
...
When initializing new accounts, the checks we need to perform are actually quite different. 

#### Native Rust

It is possible to skip owner checks for accounts owned by your own program when 
