...
In Native Rust, we would handle these checks in the body of our instruction like so:

```rs
const ESCROW_DISCRIMINATOR: u8 = 1;

let (discriminator, data) = escrow.data.borrow().split_first();
assert_eq!(&ESCROW_DISCRIMINATOR, discriminator);
assert_eq!(&crate::ID, &escrow.owner);
let escrow_data = Escrow::unpack(data);
```
