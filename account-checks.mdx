...
This is because attempting to modify memory that your program doesn't own will case the Solana runtime will throw a `InstructionError::PrivilegeEscalation` error, causing your transaction to fail. By virtue of this, if we are careful with our assumptions, it is possible to skip these checks to save .
