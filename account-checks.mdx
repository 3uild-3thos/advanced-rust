...
Instead, we actually need to check the `owner` of the account. This is because the only way for a program other than the `System Program` to take ownership of an account is through signing, proving that the account has either  has been processed by your program logic.
