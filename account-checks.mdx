...
When initializing new accounts, we need to make sure that the account is not already initialized. Naively, we may think that checking if the account is rent excempt would work, however it is not.
