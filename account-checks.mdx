...
Instead, we actually need to check the `owner` of the account the only way for a program other than the `System Program` to take ownership of the account is by PDA signing, proving it has been processed by your program logic.
