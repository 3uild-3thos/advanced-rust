...
Contrarily, when prioritizing for peak performance, if your instruction is not bound by instruction data size, it is actually preferable to pass in Sysvar accounts instead of invoking the Sysvar syscalls to save CUs. This is because the base cost of invoking Sysvar syscalls is `100 CUs`, however when paired with an efficient entrypoint like [nostd-entrypoint](https://github.com/cavemanloverboy/solana-nostd-entrypoint) or [pinocchio](https://github.com/febo/pinocchio), passing in the account can actually achieve the same result for roughly `10CUs` or so.
