...
There are two types of owner checks we should perform depending upon whether an account is already initialized or not.

#### 3.1 Initialized Accounts
...
#### 3.2 Uninitialized Accounts

When initializing new accounts, the checks we need to perform are actually quite different, as we are instead checking that the account is not already initialized.
